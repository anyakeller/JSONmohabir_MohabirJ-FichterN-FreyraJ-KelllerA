...
def ticketleap(query):
    ...
    # Final List
    fullList = []
    ticketLeap_key = keyReader.returnKey("ticketLeap")

    # Events
    link = "http://public-api.ticketleap.com/events/by/search/"+q+"?key="+ticketLeap_key+"&page_size=100"
    ...
    for i in events:
        ...
        link = "http://public-api.ticketleap.com/organizations/"+org+"/events/"+eve+"?key="+ticketLeap_key+"&page_size=100"
        u = urllib2.urlopen(link)
        response = u.read()
        data = json.loads(response)
